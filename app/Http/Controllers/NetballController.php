...
class NetballController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
        ...
        } catch (\Illuminate\Database\QueryException $e) { // Catch specific database exception
            if ($e->getCode() === '23000') { // MySQL error code for duplicate entry
                ...
                // Handle other database-related exceptions or log the error for debugging
    ...
    {
        // Modify the $content to match your data
        $content = "League: " . $request->input('league') . "\n"
            . "Team 1: " . $request->input('team1') . "\n"
            . "Team 2: " . $request->input('team2') . "\n"
            . "Stand Name: " . $request->input('standname') . "\n"
            . "Stand Number: " . $request->input('standnumber') . "\n"
            . "Date: " . now()->format('Y-m-d H:i:s');

        // Generate the barcode image
        $generator = new BarcodeGeneratorPNG();
        $barcodeImage = $generator->getBarcode($content, $generator::TYPE_CODE_128, 2, 60);
